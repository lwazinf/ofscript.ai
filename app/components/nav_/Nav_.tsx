...
import { faBoxesPacking, faBrush, faEye, faMarker, faMicrophone, faPowerOff, faScroll, faUser, faWaveSquare } from "@fortawesome/free-solid-svg-icons";
...
const Nav_ = ({}: Nav_Props) => {
  ...
  const menu_ = [{name: 'Audio', target: '/audio', icon: faMicrophone}, {name: 'Visual', target: '/visual', icon: faBrush}, {name: 'Text', target: '/text', icon: faMarker}, {name: 'split', target: '/', icon: null}, {name: 'Deliver', target: '/deliver', icon: faBoxesPacking}, {name: 'Auth', target: '/', icon: user_ ? faPowerOff : faUser}, {name: 'split', target: '/', icon: null}, {name: 'Twitter', target: 'https://x.com/ofscriptai', icon: faTwitter}, {name: 'Facebook', target: '/', icon: faFacebook}]
  return (
    <div
